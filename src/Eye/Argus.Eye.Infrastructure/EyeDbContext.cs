using Argus.Eye.Application.Interfaces;
using Argus.Eye.Domain.Entities;
using Argus.Eye.Infrastructure.Converters;
...
public class EyeDbContext : DbContext, IEyeDbContext
{
    private IDbContextTransaction? _currentTransaction;

    public EyeDbContext(DbContextOptions<EyeDbContext> options) : base(options) { }

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        base.OnModelCreating(modelBuilder);
        modelBuilder.ApplyConfigurationsFromAssembly(
            Assembly.GetAssembly(typeof(EyeDbContext)) ?? Assembly.GetExecutingAssembly());
    }
    ...
    public class EyeDbContextFactory : IDesignTimeDbContextFactory<EyeDbContext>
    {
        public EyeDbContext CreateDbContext(string[] args)
        {
            var optionsBuilder = new DbContextOptionsBuilder<EyeDbContext>();
            optionsBuilder.UseSqlite("Data Source=eye.db");

            return new EyeDbContext(optionsBuilder.Options);
        }
    }
}
